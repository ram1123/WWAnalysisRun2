...
inputFolder = "/store/user/lpcbacon/2017/";
#inputFolder = "/store/user/arapyan/"
#inputFolder = "/store/user/rasharma/FirstStep/";
#inputFolder = "/store/user/lnujj/VVjj_aQGC/Run2017/";
...
samples = [
	...
	#	aQGC Signal
    	...
    	#	DY jets
	(4435.5258,	"Fall17_DYJetsToLL_M-50_amcatnlo",	1,	0),
    	#( 4435.5258,	"DYJetsToLL_M-50_TuneCUETP8M1_13TeV-amcatnloFXFX-pythia8",	0),
	#(0.0,	"DYJetsToLL_M-50_TuneCP5_13TeV-amcatnloFXFX-pythia8_12Apr2018",	1,	0),
	#(0.0,	"WJetsToLNu_TuneCP5_13TeV-madgraphMLM-pythia8_ext1-v2", 1, 0),
	#(0.0,	"DYJetsToLL_M-50_HT-70to100_copy_cmantill",	1,	0),
...
nameDataMu = [
	...
	##"SingleMuon_Run2017E-31Mar2018-v1",
	"SingleElectron",
   ];
...
if( doMC ):
	...
	#outJDL.write("Transfer_Input_Files = "+inputlist+" "+"listTemp_"+str(samples[i][1])+"_"+str(counts)+".txt  \n");
        ...
        outJDL.write("Arguments = -n "+str(samples[i][1])+" -o WWTree_"+str(samples[i][1])+"_"+str(counts)+" -w "+str(samples[i][0])+" -no "+ str(samples[i][2]) + " -noNeg " + str(samples[i][3]) + " -lumi "+str(lumi)+" --ismc 1 -trig 0 -c lpc -f "+ "listTemp_WWTree_"+str(samples[i][1])+"_"+str(counts)+".txt " +" \n");
        outJDL.write("Queue\n");
...
if( doData ):
    for a in range(len(category)):
        for i in range(len(nameData[category[a]])):
      	    ...
      	    ##
      	    #######################################################################
      	    print("eos root://cmseos.fnal.gov find "+inputFolder+str(nameDataMu[i])+" | awk '{print \"root://cmseos.fnal.gov/\"$1}' > temp.txt")
      	    print "\n\n"
      	    os.system("eos root://cmseos.fnal.gov find "+inputFolder+str(nameDataMu[i])+" | awk '{print \"root://cmseos.fnal.gov/\"$1}' > temp.txt")
      	    tempFile = open('temp.txt','r')
      	    CountFiles = 0
      	    CountLines = 0
      	    for lines in tempFile:
      	      if CountLines % 25 == 0:
      	        newFile = open('listTemp_'+str(nameDataMu[i])+'_'+str(CountFiles)+'.txt','w')
      	        CountFiles = CountFiles + 1
      	      newFile.write(lines)
      	      CountLines = CountLines + 1
      	    ######################################################################
      	    ...
      	    #
      	    ######################################################################
      	    for counts in xrange(CountFiles+1):
               outJDL.write("Output = "+OutputLogPath+"/"+(nameData[category[a]])[i]+"_"+str(counts+1)+".stdout\n");
               outJDL.write("Error = "+OutputLogPath+"/"+(nameData[category[a]])[i]+"_"+str(counts+1)+".stdout\n");
               outJDL.write("Log = "+OutputLogPath+"/Logs/"+(nameData[category[a]])[i]+"_"+str(counts+1)+".log\n");
               #outJDL.write("Arguments = -n "+(nameData[category[a]])[i]+" -o WWTree_"+(nameData[category[a]])[i]+"_"+category[a]+"_"+str(counts)+" -w 1. -no 1. --ismc 0 -trig 1 -c lpc -f "+ "listTemp_"+str(nameDataMu[i])+"_"+str(counts)+".txt " +" \n");
               outJDL.write("Arguments = -n "+(nameData[category[a]])[i]+" -o "+(nameData[category[a]])[i]+"_"+str(counts+1)+" -w 1. -no 1. --ismc 0 -trig 1 -c lpc -f "+ "listTemp_"+str(nameDataMu[i])+"_"+str(counts+1)+".txt " +" \n");
               print("Arguments = -n "+(nameData[category[a]])[i]+" -o WWTree_"+(nameData[category[a]])[i]+"_"+category[a]+"_"+str(counts+1)+" -w 1. -no 1. --ismc 0 -trig 1 -c lpc -f "+ "listTemp_"+str(nameDataMu[i])+"_"+str(counts+1)+".txt " +" \n");
               outJDL.write("Queue\n");
