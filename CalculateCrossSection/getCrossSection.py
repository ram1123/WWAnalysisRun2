...
#
#	python runHatsTrees.py -i <InputDatPath> -o <Output>.root -t "otree"  -c 18.47
###
usage = """%prog [options]
...
parser.add_option("-o", "--outFileName", dest = "outFileName",
		  default = "Text_Output_Cross_Section.dat",
                  help = "the output root file name. It will be located within the 'output' directory" )
parser.add_option("-c", "--crossSection",  
		  help = "enter the total cross-section calculated from genProduction or from the log of gridpack")
...
inFiles = []
print "Trying to read input root files..."
#for inFile in filter(None,popen("xrdfs root://cmseos.fnal.gov/ ls -u "+options.inDir).read().split('\n')):
for inFile in filter(None,popen("eos find -f "+options.inDir).read().split('\n')):
	#print inFile
	#if "*.root" in inFile:
...
###
if not options.load:
  gSystem.CompileMacro("Calculate_CrossSection.C", "gOck")

### Now we can load our class into our pyROOT session:
gSystem.Load("Calculate_CrossSection_C")

### HATS comment:
# now we create an instance of our class
###
instance = Calculate_CrossSection(chain)
...
if not exists("output"):
  mkdir("output")
#instance.Loop()
#instance.Loop("%s" % options.outFileName,18.42)
instance.Loop("%s" % options.outFileName,Decimal(options.crossSection))
